...
=item B<--use-consistency>

Uses the file generated by the --consistency command e.g. result.bmreliability-cbt
...
GetOptions(
    ...
    "use-consistency" => \$use_consistency,
...
# use normal am-/bmreliability if "--use-consistency-output is not defined"
if (@ARGV==1) {
    my $dir=$ARGV[0];
    # print STDERR "Use files from LocARNA output directory $dir\n";
    $alnfile = "$dir/results/result.aln";
    $bmrelfile = !defined($use_consistency) ? "$dir/results/result.bmreliability" : "$dir/results/result.bmreliability-cbt";
    $amrelfile = !defined($use_consistency) ? "$dir/results/result.amreliability" : "$dir/results/result.amreliability-cbt";
} else {
    $alnfile = $ARGV[0];
    $bmrelfile = $ARGV[1];
    $amrelfile = $ARGV[2];
...
my $penalty_sum = 0; ## sum of weights of base pairs that are broken by a split between positions i-1 and i
for (my $i = 1; $i <= $seq_len; $i++) {
    ...
    my $r = defined $right{$i} ? $right{$i} : 0;


    # calculate delta values
    $delta = $position_penalty_weight * $penalty_sum;
