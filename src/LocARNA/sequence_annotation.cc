...
namespace LocARNA {
    ...
    SequenceAnnotation::is_neutral(const name_t &name) {
        for (name_t::const_iterator it=name.begin();
             name.end()!=it; ++it) {
            if (! is_neutral_char(*it) ) return false;
        }
        return true;
    }
    ...
    SequenceAnnotation::is_neutral_pos(size_t i) const {
        ...
        for (size_t k=0; k<annotation_.size(); k++) {
            if (! is_neutral_char(annotation_[k][i-1]) ) return false;
        }
        return true;
    }
    ...
    SequenceAnnotation::duplicate_names() const {
        ...
        for (size_t i=1; i<=length(); i++) {
            if (is_neutral_pos(i)) continue;

            for (size_t j=i+1; j<=length(); j++) {
                if (is_neutral_pos(j)) continue;

                if (name(i)==name(j)) return true;
            }
        }
    ...
    SequenceAnnotation::clashing_names(const AlignmentEdges &edges,
        ...
        if (annotationA.empty() || annotationB.empty()) return false;
